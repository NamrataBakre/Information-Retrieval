...
export default class SearchBarTest extends React.Component {
  constructor(props){
    ...
    onSubmit = (e) => {
      ...
      if (this.isIndia){
        this.state.filter_country.push('india')
      }
      if (this.isUSA){
        this.state.filter_country.push('usa')
      }
       if (this.isItaly){
        this.state.filter_country.push('italy')
      }

      if ( ! query ) {
        this.setState({ query, results: {}, results_tweets: {tweets:[]},  message: '' } );
      } else {
        this.setState({ query, loading: true, message: '', renderList: true, filter_country: this.state.filter_country }, () => {
